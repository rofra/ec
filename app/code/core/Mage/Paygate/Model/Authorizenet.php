...
class Mage_Paygate_Model_Authorizenet extends Mage_Payment_Model_Method_Cc
{
	...
	//	file_put_contents( "/tmp/rc", "currencycode2:".$order->getBaseCurrencyCode(). "\n", FILE_APPEND );

	$amt = $payment->getAmount();
	$multiplier = 1;
	if( $order->getBaseCurrencyCode() == "EUR" )
	    {
		$multiplier = $this->calcExchangeRate();
//		file_put_contents( "/tmp/rc", "exchange rate was $multiplier\n", FILE_APPEND );
	    }
        if($payment->getAmount() * $multiplier){
            $request->setXAmount($payment->getAmount() * $multiplier,2);
            $request->setXCurrencyCode("USD");
        }
        ...
        if (!empty($order)) {
            ...
            $request->setXPoNum($payment->getPoNumber())
                ->setXTax($order->getBaseTaxAmount() * $multiplier)
                ->setXFreight($order->getBaseShippingAmount() * $multiplier);
        }
...
function get_data($url)
{
  ...
  return $data;
	...
	try
	    {
            $cachefile = "/home/etiquett/currrate.txt";
		$mtime = filemtime( $cachefile );
		//		file_put_contents( "/tmp/rc", "should we?\n", FILE_APPEND );
		if( !file_exists( $cachefile ) || $mtime < mktime( 0,0,0 ) )
		    {
			//			file_put_contents( "/tmp/rc", "we need to do it\n", FILE_APPEND );
			$subject = $this->get_data( "http://www.x-rates.com/d/USD/EUR/graph120.html" );
//			file_put_contents( "/tmp/rc", "after\n", FILE_APPEND );
			$pattern = '/latest.*<\/font>/';//  (Feb 16)<br>1.2982</font>
			preg_match($pattern, $subject, $matches);
			$val = $matches[0];
			$pos = strpos( $val,  "<br>" );
			$s = split( "<br>", $val );
			$val = strip_tags( $s[count($s)-1] );
			if( $val > 0 )
			    {
				//				file_put_contents( "/tmp/rc", "val was $val\n", FILE_APPEND );
				$h = fopen( $cachefile, "w" );
				fwrite( $h, $val );
				fclose( $h );
			    }
		    }
		$val = file_get_contents( $cachefile );
		if( $val )
		    return $val;
        } catch (Exception $e) {
//	    file_put_contents( "/tmp/rc", "error:" . $e->getCode() . ": " . $e->getMessage(), FILE_APPEND );
	}
	return 1.3109;
    }
