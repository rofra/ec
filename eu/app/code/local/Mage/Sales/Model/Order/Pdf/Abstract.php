...
//	    file_put_contents( "/tmp/rc", $image );
    ...
    protected function insertAddress(&$page, $store = null)
    {
        ...
        $page->drawLine(125, 825 - $this->startheight, 125, 790 - $this->startheight);

        $page->setLineWidth(0);
        $this->y = 820 - $this->startheight;
    ...
    protected function insertOrder(&$page, $obj, $putOrderId = true)
    {
        ...
        $page->drawRectangle(25, 790 - $this->startheight, 570, 755 - $this->startheight);
        ...
        if ($putOrderId) {
            $page->drawText(Mage::helper('sales')->__('Order # ').$order->getRealOrderId(), 35, 770 - $this->startheight, 'UTF-8');
        }
        //$page->drawText(Mage::helper('sales')->__('Order Date: ') . date( 'D M j Y', strtotime( $order->getCreatedAt() ) ), 35, 760, 'UTF-8');
        $page->drawText(Mage::helper('sales')->__('Order Date: ') . Mage::helper('core')->formatDate($order->getCreatedAtStoreDate(), 'medium', false), 35, 760 - $this->startheight, 'UTF-8');

        $page->setFillColor(new Zend_Pdf_Color_Rgb(0.93, 0.92, 0.92));
        $page->setLineColor(new Zend_Pdf_Color_GrayScale(0.5));
        $page->setLineWidth(0.5);
        $page->drawRectangle(25, 755 - $this->startheight, 275, 730 - $this->startheight);
        $page->drawRectangle(275, 755 - $this->startheight, 570, 730 - $this->startheight);
        ...
        $page->setFillColor(new Zend_Pdf_Color_GrayScale(0));
        $this->_setFontRegular($page);
        $page->drawText(Mage::helper('sales')->__('SOLD TO:'), 35, 740 - $this->startheight , 'UTF-8');

        if (!$order->getIsVirtual()) {
            $page->drawText(Mage::helper('sales')->__('SHIP TO:'), 285, 740 - $this->startheight , 'UTF-8');
        }
        else {
            $page->drawText(Mage::helper('sales')->__('Payment Method:'), 285, 740 - $this->startheight , 'UTF-8');
        }
        ...
        $page->setFillColor(new Zend_Pdf_Color_GrayScale(1));
        $page->drawRectangle(25, 730 - $this->startheight, 570, $y - $this->startheight);
        $page->setFillColor(new Zend_Pdf_Color_GrayScale(0));
        $this->_setFontRegular($page);
        $this->y = 720 - $this->startheight;
